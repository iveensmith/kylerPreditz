...
const RESULT_LETTER: Record<"W" | "D" | "L", string> = { W: "bg-win", D: "bg-faint", L: "bg-loss" };
  ...
  return (
    <div>
      ...
      {stats.last6Results.length === 0 ? (
        <p className="text-xs text-faint italic">No recent results tracked yet.</p>
      ) : (
          ...
          {stats.last6Results.map((r, i) => {
            ...
            return (
              <li key={i} className="flex items-center gap-2 text-xs">
                ...
                <span className="text-muted">{r.venue === "home" ? "vs" : "@"}</span>
